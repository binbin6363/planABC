...
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(uint8_t &data)
{
	data = read_uint8();
	return;
}
...
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(uint16_t &data)
{
...
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(uint32_t &data)
{
...
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(uint64_t &data)
{
	uint32_t hi, lo;
	(*this) >> hi >> lo;
...
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(String &data)
{
...
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(std::string& data)
{
	String d = read_string();

	data.clear();
	
	data = std::string(d.buf,d.len);
...
template<typename HEAD>
Binary_Read_Stream<HEAD>& Binary_Read_Stream<HEAD>::operator >>(std::string& data)
{
	String d = read_string();
...
*/
template<typename HEAD>
void Binary_Read_Stream<HEAD>::serialize(Binary &data)
{
...
template<typename HEAD>
const char* Binary_Read_Stream<HEAD>::get_cur()
{
	return m_buf + m_pos;
}
...
String Binary_Read_Stream<HEAD>::read_string()
{
	...
	uint32_t len = *(uint32_t*)(m_buf + m_pos);
	len = ntohl(len);

	// read str
	if ((len == 0) || ((m_len - m_pos - 4) < len) || (m_buf[m_pos + 4 + len - 1] != 0)) {
		m_good = false;
		return str;
	}

	str.buf = m_buf + m_pos + 4;
	str.len = len - 1;
...
Binary Binary_Read_Stream<HEAD>::read_binary()
{
	...
	// read str
	if ((len == 0) || ((m_len - m_pos - 4) < len)) {
		m_good = false;
		return bin;
	}

	bin.buf = m_buf + m_pos + 4;
	bin.len = len;
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(uint8_t data)
{
	write_uint8(data);
	return;
}
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(uint16_t data)
{
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(uint32_t data)
{
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(uint64_t data)
{
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(const String& data)
{
	if (data.buf == NULL) {
		m_good = false;
		return;
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(const std::string& data)
{
	*this<<String(data.c_str(),data.size());
	return;
}

template<typename HEAD>
Binary_Write_Stream<HEAD>& Binary_Write_Stream<HEAD>::operator <<(const std::string& data)
{
	*this<<String(data.c_str(),data.size());
	return *this;
}
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(const char *data)
{
	if (data == NULL) {
		m_good = false;
		return ;
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(const Binary& data)
{
	if (data.buf == NULL) {
		m_good = false;
		return ;
...
template<typename HEAD>
void Binary_Write_Stream<HEAD>::serialize(Binary_Write_Stream& (*pfn)(Binary_Write_Stream&))
{	
	pfn(*this);
	return;
}
...
void Binary_Write_Stream<HEAD>::write_string(const String& data)
{
	...
	if ((m_len - m_pos) < (4 + len + 1)) {
		m_good = false;
		return;
	}

	*(uint16_t*)(m_buf + m_pos) = htons(len + 1);
	memcpy(m_buf + m_pos + 4, data.buf, len);
	m_buf[m_pos + 4 + len] = 0;
...
void Binary_Write_Stream<HEAD>::begin_binary()
{
	...
	*(uint32_t*)(m_buf + m_binary_len_pos) = htonl(m_binary_len);
...
void Binary_Write_Stream<HEAD>::write_binary(const Binary& data)
{
	...
	{
		// 写入一个完整数据块
		if ((m_len - m_pos) < (4 + data.len)) {
			m_good = false;
			return;
		}

		*(uint32_t*)(m_buf + m_pos) = htonl(data.len);
		memcpy(m_buf + m_pos + 4, data.buf, data.len);
