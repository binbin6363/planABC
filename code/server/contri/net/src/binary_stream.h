...
public:
	...
	Binary_Read_Stream& operator >>(std::string& data);
...
public:
	...
	Binary_Write_Stream& operator <<(const std::string& data);

	//! 写入数据块
	//! @param data 写入数据
	//! @return 二进制写入流
	void serialize(const Binary& data);
	Binary_Write_Stream& operator <<(const Binary& data);

	//! 执行流操作符
	//! @param pfn 流操作符函数指针
	//! @return 二进制写入流
	void serialize(Binary_Write_Stream& (*pfn)(Binary_Write_Stream&));
