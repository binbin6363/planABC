...
//D = false 数据包从右往左
  ...
  class BinInputPacket : public BinPacket<D>
  {
  	public:
		...
		friend BinInputPacket& operator>>(BinInputPacket& in, uint64_t& value)
		{
  			if(!in.m_good || in.m_eof) return in;
  			if(D){
  				if(in.m_end - in.m_current < static_cast<int32_t>(sizeof(value))){
  					in.m_good = false;
  					return in;
  				}
  			}else{
  				if(in.m_current - in.m_begin < static_cast<int32_t>(sizeof(value))){
  					in.m_good = false;
  					return in;
  				}
  			}
  			...
  			if(D) 
			{
				unres64 = (*reinterpret_cast<uint64_t*>(in.m_current));
				in.m_current += 8;
				if(in.m_current == in.m_end) in.m_eof = true;
  			}
  			else  
			{
				unres64 = (*reinterpret_cast<uint64_t*>((in.m_current-8)));
				in.m_current -= 8;
				if(in.m_current == in.m_begin) in.m_eof = true;
  			}
    			uint32_t low = (uint32_t) (unres64 & 0x00000000FFFFFFFFLL);
    			uint32_t high = (uint32_t) ((unres64 & 0xFFFFFFFF00000000LL) >> 32);
    			low = ntohl(low);
    			high = ntohl(high);
      ...
      friend BinInputPacket& operator>>(BinInputPacket& in, PSTR& value)
      {
        ...
        uint32_t len;
  			in>>len;
  			if( in.m_end - in.m_current < len || value.len < len ){
  				in.m_good = false;
  				return in;
  			}
        value.len = len;
  ...
  class BinOutputPacket : public BinPacket<D>
  {
  	public:
		...
		friend BinOutputPacket& operator<<(BinOutputPacket& out, uint64_t value)
		{
			if(!out.m_good || out.m_eof) return out;

  			if(D){
  				if(out.m_end - out.m_current < static_cast<uint32_t>(sizeof(value))){
  					out.m_good = false;
  					return out;
  				}
  			}else{
  				if(out.m_current - out.m_begin < static_cast<int32_t>(sizeof(value))){
  					out.m_good = false;
  					return out;
  				}
  			}
			...
			low = htonl(low);
			high = htonl(high);
			unres64 = (uint64_t) high + (((uint64_t) low) << 32);
			if(D) 
			{
				*reinterpret_cast<uint64_t*>(out.m_current) = unres64;
				out.m_current += 8;
				if( out.m_current == out.m_end ) out.m_eof = true;
			}
			else 
			{
				*reinterpret_cast<uint64_t*>((out.m_current-8)) = unres64;
				out.m_current -= 8;
				if( out.m_current == out.m_begin ) out.m_eof = true;
			}
  		...
  		friend BinOutputPacket& operator<<(BinOutputPacket& out, BNSTR value)
  		{
            ...
            {
                out << value.len;
                out.write(value.data, value.len);
            }
            else
            {
                out.write(value.data, value.len);
