...
int InitConfig(const std::string &cfg_path, Config& config)
{
    ...
    // load ldb server
    if (!xml_parser.LoadServerInfo(LDB_SERVER_NAME, config.ldb_srv_infos, config.ldb_service_name, default_info))
    {
        ret = -1;
        fprintf(stderr, "Load ldb ServerInfo failed.");
        break;
    }
    ...
    {
        ret = -1;
        fprintf(stderr, "Load redis ServerInfo failed.");
        break;
    }
    ONCE_LOOP_LEAVE
    return ret;
