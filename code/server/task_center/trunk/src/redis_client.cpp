...
{
    snprintf(keystr, KEY_LENGTH, "%s%u", UID_PREFIX, hashid);
    return keystr;
}
...
int RedisClient::SyncConnectRedis(uint32_t max_net_buf)
{
    ...
    if ((NULL == pRedisSendBuff) && (NULL == aryMsgBuff))
    {
...
//	TimeCounterAssistant tca("connect redis, SyncConnectRedis");
	...
	uint32_t port = StringUtil::strtou32(serv_info_.server_port);
	// 单位是s
    ...
    ret = setRedisTimeOut(tTimeout);
	if (0 != ret)
	{
		LOG(ERROR)("set redis timeout failed, err code:%d, [%s]", ret, serv_info_.print());
		connect_fail();
		break;
	}
...
{
    LOG(ERROR)("redis lost connect, start to reconnect redis server. peer addr info %s:%s", serv_info_.server_ip.c_str(), serv_info_.server_port.c_str());    
    bConnected_ = false;
	if(NULL != pRdsContext)
	{
		redisFree(pRdsContext);
...
void RedisClient::handle_timeout(int id,void *userData)
{
    ...
    {
        LOG(INFO)("reconnect redis server, addr:%s:%s", serv_info_.server_ip.c_str(), serv_info_.server_port.c_str());
    ...
    if (bConnected_ && (cur_time >= (last_time_ + serv_info_.heart_beat_interval)) )
    {
        keepalive();
    }

    // 定时器，仅此处注册
    TimeoutManager::Instance()->RegisterTimer(this, MAKE_SECOND_INTERVAL(1), NULL);
}
...
void RedisClient::keepalive()
{
    ...
    {
        LOG(ERROR)("redis keepalive failed, redis is not connected, peer addr info %s:%s", serv_info_.server_ip.c_str(), serv_info_.server_port.c_str());
        return;
    }
    ...
    {
        last_time_ = time(NULL);
        LOG(INFO)("[beat] redis keep alive success, peer addr info %s:%s", serv_info_.server_ip.c_str(), serv_info_.server_port.c_str());
    }
    if (NULL != reply)
...
//    TimeCounterAssistant tca("visit redis, GetTaskIds");
    ...
    const char *key = MakeTaskIdsKey(uid);
    LOG(DEBUG)("GetTaskIds, hash id:%u, get key:%s", uid, key);
...
//    TimeCounterAssistant tca("visit redis, CheckTaskUsedOut");
    ...
    {
        const char *key = MakeTaskLimitKey(*iter);
...
int RedisClient::DecrTaskNum(const uint64_t &hash_id, const uint64_t &task_id)
{
    redisReply* reply = NULL;
    ...
    // select db
    if (!SelectRedisDb(hash_id, RDS_DB_SIZE))
    {
        return -1;
    }

    const char *key = MakeTaskLimitKey(task_id);
    LOG(DEBUG)("DecrTaskNum, task id:%lu, get key:%s", task_id, key);
    redisAppendCommand(pRdsContext, "DECR %s", key);
    ...
    if (REDIS_OK == ret_value)
    {
        int cur_index = reply->integer;
        if (cur_index < 0)
        {
            ret_value = ERR_TASK_OVERFLOW;
            LOG(INFO)("task overflow, task id:%lu, left num:%ld", task_id, cur_index);
        } else {
            LOG(INFO)("task can used, task id:%lu, left num:%ld", task_id, cur_index);
        }
    }
    else
    {
        LOG(ERROR)("DecrTaskNum, redis error, ret:%d", ret_value);
    }
    if (reply != NULL) {
        freeReplyObject(reply);
        reply = NULL;
...
int RedisClient::RemoveTaskIds(uint32_t uid, const uint64_t &id)
{
//    TimeCounterAssistant tca("visit redis, RemoveTaskIds");
    ...
    // select db
    if (!SelectRedisDb(id, RDS_DB_SIZE))
    {
        return -1;
    }

    const char *key = MakeTaskIdsKey(uid);
    LOG(DEBUG)("RemoveTaskIds, task id:%lu, get key:%s", id, key);
    ...
    if (REDIS_OK == ret_value)
    {
        if (reply->integer != id)
        {
            LOG(ERROR)("RPOP task id mistake, key:%s, uid:%u, target task id:%lu, RPOP taskid:%lu."
                , key, uid, id, reply->integer);
        } else {
            LOG(INFO)("RPOP ok, key:%s, uid:%u, task id:%lu", key, uid, id);
        }
    }
    else
    {
        LOG(ERROR)("RemoveTaskId, redis error, ret:%d, key:%s, uid:%u, task id:%lu"
            , ret_value, key, uid, id);
    }
    if (reply != NULL) {
        freeReplyObject(reply);
        reply = NULL;
...
int RedisClient::AddTaskToUser(uint32_t uid, const uint64_t &task_id, int expire)
{
    redisReply* reply = NULL;
    ...
    string key = MakeTaskIdsKey(uid);
    string value = StringUtil::u64tostr(task_id);
    LOG(DEBUG)("AddTaskToUser, key:%s, value:%s", key.c_str(), value.c_str());
    
    redisAppendCommand(pRdsContext, "LPUSH %s %s", key.c_str(), value.c_str());
    ...
    if (REDIS_OK == ret_value)
    {
        if (0 == reply->integer)
        {
            LOG(WARN)("push task to user list failed, key:%s, uid:%u, push num:%d", key.c_str(), uid, reply->integer);
        } else {
            LOG(INFO)("push task to user list ok, key:%s, uid:%u, push num:%d", key.c_str(), uid, reply->integer);
        }
    }
    else
    {
        LOG(ERROR)("AddTaskToUser, redis error, ret:%d, key:%s"
            , ret_value, key.c_str());
        ret_value = ERR_REDIS_ERROR;
    }
    if (reply != NULL) {
        freeReplyObject(reply);
        reply = NULL;
...
bool RedisClient::ExistKey(const uint64_t &id, const string &key)
{
    redisReply* reply = NULL;
    ...
    LOG(DEBUG)("ExistKey, id:%lu, check exists key:%s", id, key.c_str());
    redisAppendCommand(pRdsContext, "EXISTS %s", key.c_str());
    ...
    if (REDIS_OK == ret_value)
    {
        if (reply->integer != 0)
        {
            LOG(INFO)("key exist, key:%s.", key.c_str());
            ret = true;
        } else {
            LOG(INFO)("key not exist, key:%s.", key.c_str());
            ret = false;
        }
    }
    else
    {
        LOG(ERROR)("ExistKey, redis error, ret:%d, key:%s", ret_value, key.c_str());
        ret = false;
    }
    if (reply != NULL) {
        freeReplyObject(reply);
...
// expire, 过期时间，单位是秒
int RedisClient::SetKeyValue(const uint64_t &id, const string &key, const string &value, uint32_t expire)
{
    redisReply* reply = NULL;
    ...
    LOG(DEBUG)("SetKeyValue, key:%s, value:%s", key.c_str(), value.c_str());
    if (0 != expire) {
        LOG(DEBUG)("setex key, key:%s, expire:%u", key.c_str(), expire);
        redisAppendCommand(pRdsContext, "SETEX %s %u %s", key.c_str(), expire, value.c_str());
    } else {
        redisAppendCommand(pRdsContext, "SET %s %s", key.c_str(), value.c_str());
    }
    ...
    if (REDIS_OK == ret_value)
    {
        if (0 == StringUtil::strcasecmp(reply->str, "ok"))
        {
            LOG(INFO)("set key ok, key:%s", key.c_str());
            ret_value = 0;
        } else {
            LOG(ERROR)("set key failed, key:%s", key.c_str());
            ret_value = ERR_REDIS_ERROR;
        }
    }
    else
    {
        LOG(ERROR)("SetKeyValue, redis error, ret:%d, key:%s"
            , ret_value, key.c_str());
        ret_value = ERR_REDIS_ERROR;
    }
    if (reply != NULL) {
