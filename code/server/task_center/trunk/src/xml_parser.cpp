...
//    fprintf(stdout, "LoadSystemInfo succeed. servicename:%s, ip:%s, port:%s, ttl:%u, ftl:%u"\
//        ", bufsize:%u, timeout:%u, libco_timeout:%u"
//        , config.sys_name.c_str(), config.sys_ip.c_str(), config.sys_port.c_str()
//        , config.sys_ttl, config.sys_ftl, config.sys_buf_size
//        , config.sys_timeout, config.sys_libco_timeout);
