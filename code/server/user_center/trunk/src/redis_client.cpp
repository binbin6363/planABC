...
#ifndef CHECK_REDIS_STATUS
#define CHECK_REDIS_STATUS()\
{\
    if(!bConnected_) {\
		LOG(ERROR)("redis server is not connected.");\
        return -1;\
    }\
    if (pRdsContext == NULL) { \
		LOG(ERROR)("redis content is null.");\
        return -2;\
	}\
}
#endif
...
int RedisClient::SelectRedisDb(uint64_t hash_id, uint32_t region)
{
    int ret = 0;
    redisReply* reply = NULL;
ONCE_LOOP_ENTER
...
const char *RedisClient::GetTaskLimitKey(uint64_t taskid)
{
    snprintf(keystr, KEY_LENGTH, "%s%lu", TASK_LIMIT_PREFIX, taskid);
    return keystr;
}

const char *RedisClient::GetUserIdKey()
{
...
//    TimeCounterAssistant tca("visit redis, RemoveTaskIds");
    ...
    if (REDIS_OK == ret_value)
    {
        if (reply->integer != id)
        {
            LOG(ERROR)("RPOP task id mistake, key:%s, uid:%u, target task id:%lu, RPOP taskid:%lld."
...
int RedisClient::GenerateUserId(uint32_t &uid)
{
    redisReply* reply = NULL;
    ...
    const char *key = GetUserIdKey();
    LOG(DEBUG)("GenerateUserId, get key:%s", key);
    redisAppendCommand(pRdsContext, "INCR %s", key);

    // fetch result
    ret_value = redisGetReply(pRdsContext, (void **)(&reply));
    if (REDIS_OK == ret_value)
    {
        uid = (uint32_t)reply->integer;
        if (uid < MIN_GLOBAL_USER_ID) {
            LOG(ERROR)("generate user id failed, id(%u) < min id(%u)", uid, MIN_GLOBAL_USER_ID);
            ret_value = -1;
        } else {
            LOG(INFO)("GenerateUserId ok, key:%s, uid:%u", key, uid);
        }
    }
    else
    {
        ...
        LOG(ERROR)("GenerateUserId failed, redis error, ret:%d, key:%s"
            , ret_value, key);
    }
    if (reply != NULL) {
        freeReplyObject(reply);
