...
// redis operate
public:
	const char *GetTaskLimitKey(uint64_t taskid);
	...
	int GenerateUserId(uint32_t &uid);
...
private:
	const char *GetTaskIdsKey(uint32_t hashid);
	const char *GetUserIdKey();
...
private:
    ...
    // 选择db
    int SelectRedisDb(uint64_t hash_id, uint32_t region);
