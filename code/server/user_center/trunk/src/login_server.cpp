...
int InitConfig(const std::string &cfg_path, Config& config)
{
    ...
    // user
    if (!xml_parser.LoadUserInfo(config))
    {
        ret = -1;
        fprintf(stderr, "Load user ServerInfo failed.");
        break;
    }
    ...
    // load dbp server
    if (!xml_parser.LoadServerInfo(DBP_SERVER_NAME, config.dbp_srv_infos, config.dbp_service_name, default_info))
    {
        ret = -1;
        fprintf(stderr, "Load Dbp ServerInfo failed.");
        break;
    }

    // load rds server
    if (!xml_parser.LoadServerInfo(RDS_SERVER_NAME, config.rds_srv_infos, config.rds_service_name, default_info))
    {
        ret = -1;
        fprintf(stderr, "Load Rds ServerInfo failed.");
        break;
    }
...
int main(int argc, char *argv[])
{
    ...
    LoginApp::Instance()->RegistFactory(config);

    fprintf(stdout, "open config.\n");
    // todo: 打开连接
    if (LoginApp::Instance()->open(config) != 0)
    {
    ...
    ret = LoginApp::Instance()->run_service();
    if (0 != ret)
    {
    ...
    LoginApp::Instance()->UnregistFactory(config);
    ONCE_LOOP_LEAVE
    return ret;
}
