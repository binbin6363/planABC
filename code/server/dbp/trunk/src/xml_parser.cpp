...
//bool XmlParser::LoadDbInfo(Config &config)
//{
//    bool ret = true;
//    ONCE_LOOP_ENTER
//    uint32_t buf_size = 10;
//    TiXmlElement *db_element = root_element_->FirstChildElement("mysql");
//    CHECK_ERROR_BREAK((db_element == NULL), false, "LoadDbInfo get mysql element falied");

//    GetIntegerValue(db_element, "id", config.sys_id);
//    GetStringValue(db_element, "servicename", config.sys_name);
//    GetStringValue(db_element, "ip", config.sys_ip);
//    GetStringValue(db_element, "port", config.sys_port);
//    GetIntegerValue(db_element, "ttl", config.sys_ttl);
//    GetIntegerValue(db_element, "ftl", config.sys_ftl);
//    GetIntegerValue(db_element, "buf_size", config.sys_buf_size, buf_size);
//    config.sys_buf_size *= _1K;
//    
//    LOG(INFO)("LoadDbInfo succeed. id:%u, servicename:%s, ip:%s, port:%s, ttl:%u, ftl:%u"\
//        ", bufsize:%u, timeout:%u, libco_timeout:%u, co max:%u.\n"
//        , config.sys_id, config.sys_name.c_str(), config.sys_ip.c_str(), config.sys_port.c_str()
//        , config.sys_ttl, config.sys_ftl, config.sys_buf_size
//        , config.sys_timeout, config.sys_libco_timeout, config.sys_max_co_num);
...
// 加载简单通用的服务信息
bool XmlParser::LoadCommonServiceInfo(const char *section
, string &ip
, string &port
, uint32_t &timeout
, string &name)
{
    ...
    if (section_element == NULL)
    {
        LOG(ERROR)("LoadCommonServerInfo get %s element falied", section);
        return false;
    }
    GetStringValue(section_element, "servicename", name, "UNKNOWN_SERVICE");
    GetStringValue(section_element, "ip", ip, "0.0.0.0");
    GetStringValue(section_element, "port", port, "0");
    GetIntegerValue(section_element, "timeout", timeout, 10);
    LOG(INFO)("LoadCommonServiceInfo, section:%s, ip:%s, port:%s, tmeout:%u, name:%s."
        , section, ip.c_str(), port.c_str(), timeout, name.c_str());
    return true;
}


bool XmlParser::LoadErrRedirectFile(Config &config)
{
    bool ret = true;
    ONCE_LOOP_ENTER
    TiXmlElement *redict_element = root_element_->FirstChildElement("stderrredirect");
    CHECK_WARN_BREAK((redict_element == NULL), false, "LoadErrRedirectFile get stderrredirect element falied");
    GetStringValue(redict_element, "file", config.err_file);
    ONCE_LOOP_LEAVE
    LOG(INFO)("done LoadErrRedirectFile:%s", config.err_file.c_str());
    return true;
}


int XmlParser::ReloadConfig(Config &config)
{
    int ret = 0;

    ret = InitConfig(cfg_path_, config);
