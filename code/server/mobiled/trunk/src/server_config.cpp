...
name_id server_maps[] = {
    ...
    , name_id(TASK_CENTER_SERVER_NAME, TASK_CENTER_SERVER)
    , name_id(DBP_SERVER_NAME, DBP_SERVER)
};
	...
	, group_read_req_overtime_( 3 )
{
    ...
    max_buffer_size.insert(make_pair(TASK_CENTER_SERVER_NAME, 1024*1024));
    max_buffer_size.insert(make_pair(DBP_SERVER_NAME, 20*1024));
...
//    load_server_info(STATUS_SERVER_NAME, vtStatusInfo_, tGeneralInfo_);
    // user center service
    load_server_info(USER_CENTER_SERVER_NAME, vtUserCenterInfo_, tGeneralInfo_);
    // task center service
    load_server_info(TASK_CENTER_SERVER_NAME, vtTaskCenterInfo_, tGeneralInfo_);
    // dbp service
    load_server_info(DBP_SERVER_NAME, vtDbpInfo_, tGeneralInfo_);
...
const vector<Server_Info> &ServerConfig::server_info(const char *server_name) const
{
    ...
    switch (id)
        ...
        case USER_CENTER_SERVER:
            return vtUserCenterInfo_;
        case TASK_CENTER_SERVER:
            return vtTaskCenterInfo_;
        case DBP_SERVER:
            return vtDbpInfo_;
        default:
            return vtNullInfo_;    
