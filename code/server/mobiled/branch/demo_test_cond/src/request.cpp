...
FrontLcsBeatRequest::FrontLcsBeatRequest(BinInputPacket<> &inpkg)
    : inpkg_(inpkg)
    , trans_id_(0)
    , time_(0)
{
...
FrontLcsBeatRequest::~FrontLcsBeatRequest()
{
}

int FrontLcsBeatRequest::decode()
{
	int ret = 0;
//	FrontRequest::decode();
    
	inpkg_.offset_head(sizeof(LCSHEADER));
	inpkg_.get_head(header_);
	LOG(INFO)("client request head [%s]", header_.print());
	Json::Value root;
	string input_str(inpkg_.getCur()+4, inpkg_.remainLength()-4);
	LOG(DEBUG)("user beat request:%s", input_str.c_str());
	reader_.parse(input_str, root);
...
LCSHEADER FrontLcsBeatRequest::get_head()
{
    return header_;
}

FrontLcsBeatResult::FrontLcsBeatResult()
{
	data_ = ServerApp::Instance()->get_buffer();
...
int FrontLcsBeatResult::encode_body(const char *data, uint32_t data_len)
{
	int ret = 0;
	if (NULL == data_) {
		LOG(ERROR)("encode result body failed, result data is null.");
	    ...
	    int body_len = htonl(data_len);
	    memcpy(data_ + sizeof(LCSHEADER), &body_len, 4);
		memcpy(data_ + sizeof(LCSHEADER)+4, data, data_len);
		data_len_ = data_len + sizeof(LCSHEADER) + 4;
		LOG(DEBUG)("encode result body ok, encode data len:%u", data_len);
	}
	return ret;
}

int FrontLcsBeatResult::encode_head()
{
	int ret = 0;
	if (NULL == data_) {
		LOG(ERROR)("encode result head failed, data is null.");
	...
	header_.len = data_len_;
	LCSHEADER head = header_;
	head.len = htonl(header_.len);
	head.cmd = htonl(header_.cmd);
	head.seq = htonl(header_.seq);
	memcpy(data_, (void *)&head, sizeof(LCSHEADER));
	LOG(DEBUG)("encode result head ok, head:%s, encode head len:%u", 
	header_.print(), (uint32_t)sizeof(LCSHEADER));

	return ret;
}

void FrontLcsBeatResult::set_head(const LCSHEADER &header)
{
    header_ = header;
}

const LCSHEADER &FrontLcsBeatResult::get_head()
{
    return header_;
}


char *FrontLcsBeatResult::data()
{
	return data_;
}

int FrontLcsBeatResult::byte_size()
{
    return data_len_;
}
