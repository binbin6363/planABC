...
void stop()
{
    LOG(INFO)("[ RECV CMD stop cmd,  leveldb must stop........]");
    sleep(1);
    exit(0);
}
...
void reload_cfg()
{
    ...
    bool ret = utils::ServerConfig::Instance()->load_file(utils::ServerConfig::Instance()->file_path());
    if (!ret) {
        LOG(ERROR)("load config file failed.");
        return ;
    }
    ret = utils::ServerConfig::Instance()->load_config();
    if (!ret) {
...
static void readFileList(const char *basePath, strvec& filelist)
{
    DIR *dir;
    struct dirent *ptr;
    char filepath[1024];
    ...
    while ((ptr=readdir(dir)) != NULL)
    {
        if(strcmp(ptr->d_name,".") == 0 || strcmp(ptr->d_name,"..")==0)    ///current dir OR parrent dir
            continue;
        if(ptr->d_type == 8)    /// just collect file
        {
...
static void checkCreateDir(const std::string &dir)
{
    //如果目录不存在，创建
...
static void generateTargetFiles(strvec &deslist, const strvec& srclist)
{
    svciter iter = srclist.begin();
    svciter iter_end = srclist.end();
    string srcpath = "";
    string despath = "";
    string filename = "";
    string dbpath = utils::ServerConfig::Instance()->dbpath();
    string dumpdir = dbpath + "/dumpdir";
    checkCreateDir(dumpdir);
    for (; iter != iter_end; ++iter)
    {
        srcpath = *iter;
        filename = srcpath.substr(srcpath.rfind("/")+1);
        despath = dbpath + "/dumpdir/" + filename + ".dump";
        deslist.push_back(despath);
    }
}
...
void dumpleveldb()
{
    ...
    string dbpath = utils::ServerConfig::Instance()->dbpath();
    strvec filelist;
    filelist.reserve(VEC_DEFAULT_SIZE);
    readFileList(dbpath.c_str(), filelist);
    strvec targetfiles;
    targetfiles.reserve(VEC_DEFAULT_SIZE);
    generateTargetFiles(targetfiles, filelist);
    int length = filelist.size();
    for (int i = 0; i < length; i++) 
    {
        ok &= leveldb::DumpFile(env, filelist[i], targetfiles[i]);
    }
    LOG(INFO)("dump leveldb, bool ret:%u", ok);
...
{
    string dbpath = utils::ServerConfig::Instance()->dbpath();
    std::string dumpdir = dbpath + "/dumpdir";
    checkCreateDir(dumpdir);
    bool ret = leveldb::DumpProperty(dumpdir);
    LOG(INFO)("dump property, bool ret:%u", ret);
}

// 这个接口还是废掉吧，不安全，会导致进程挂掉。
void restart_db_engine()
{
//    int ret = LeveldbEngine::inst().reStartDb();
//    LOG(INFO)("restart leveldb engine, ret:%u", ret);
}
...
void UdpCmdSession::initCmd()
{
    ...
    m_cmd.insert(FUNMAP::value_type("STOP LEVELDB",stop));
    ...
    m_cmd.insert(FUNMAP::value_type("DUMP_LDB", dumpleveldb));
    m_cmd.insert(FUNMAP::value_type("DUMP_PROPERTY", dump_property));
...
//    m_cmd.insert(FUNMAP::value_type("RELOAD_MAINTENANCE",reload_maintenance));
	...
	m_cmd.insert(FUNMAP::value_type("RESTART_DB",restart_db_engine));
